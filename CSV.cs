...
namespace ScriptStack
{
    public class CSV : Model
    {
        ...
        public CSV()
        {
            ...
            routines.Add(new Routine((Type)null, "csv.parse", (Type)null, typeof(string), typeof(bool), "Erstelle eine CSV-Tabelle aus einem String. params: csv, sep, hasheader"));
            routines.Add(new Routine((Type)null, "csv.get", (Type)null, (Type)null, "Lese einen CSV-Wert/Row per CSV-Path."));
            routines.Add(new Routine((Type)null, "csv.set", (Type)null, (Type)null, (Type)null, "Setze einen CSV-Wert per CSV-Path."));

            routines.Add(new Routine(typeof(int), "csv.row_count", (Type)null, "Anzahl Zeilen."));
            routines.Add(new Routine(typeof(int), "csv.col_count", (Type)null, "Anzahl Spalten (max)."));
            routines.Add(new Routine(typeof(ScriptStack.Runtime.ArrayList), "csv.headers", (Type)null, "Gibt Header als ArrayList zurück (leer wenn keine Header)."));
            routines.Add(new Routine(typeof(bool), "csv.has", (Type)null, (Type)null, "Prüft ob CSV-Path existiert (und Wert != null)."));

            routines.Add(new Routine((Type)null, "csv.iter", (Type)null, "Iterator über Zeilen (CsvTable oder raw csv string)."));
            routines.Add(new Routine(typeof(bool), "csv.next", (Type)null, "Iterator: nächste Zeile. true wenn vorhanden."));
            routines.Add(new Routine(typeof(int), "csv.index", (Type)null, "Iterator: aktueller Zeilenindex."));
            routines.Add(new Routine((Type)null, "csv.value", (Type)null, "Iterator: aktuelle Zeile (ArrayList)."));

            routines.Add(new Routine((Type)null, "csv.table", (Type)null, "Konvertiert ArrayList -> CsvTable (Rows oder Object-Style)."));
            routines.Add(new Routine(typeof(string), "csv.string", (Type)null, "Konvertiert CsvTable/ArrayList -> CSV-String."));
        ...
        {
            if (routine == "csv.parse")
            {
                // csv.parse(csvString, [hasHeader=true], [delimiter=","], [culture="invariant"])
                string csv = (string)parameters[0];
                string delimiter = (string)parameters[1];
                bool hasHeader = (bool)parameters[2];
            ...
            if (routine == "csv.row_count")
            {
                var table = CoerceToTable(parameters[0]);
                return table.Rows.Count;
            }

            if (routine == "csv.col_count")
            {
                var table = CoerceToTable(parameters[0]);
                return table.GetMaxColCount();
            }

            if (routine == "csv.headers")
            {
                var table = CoerceToTable(parameters[0]);
            ...
            if (routine == "csv.index")
            {
                var it = (CsvIterator)parameters[0];
                return it.CurrentIndex;
            }

            if (routine == "csv.value")
            {
                var it = (CsvIterator)parameters[0];
                return it.CurrentRow;
            }

            if (routine == "csv.table")
            {
                // csv.table(ArrayList rowsOrObject, [ArrayList headers], [delimiter], [culture], [hasHeader])
            ...
            if (routine == "csv.string")
            {
                // csv.string(CsvTable OR ArrayList, [headers], [delimiter], [culture], [hasHeader])
                object? p0 = parameters[0];

                if (p0 is CsvTable t0)
                    return t0.ToCsvString();

                // Sonst: über csv.table bauen
                ScriptStack.Runtime.ArrayList? headers = null;
                if (parameters.Count > 1 && parameters[1] is ScriptStack.Runtime.ArrayList h) headers = h;
