...
function main() {
	...
	var f = std.fopen("test.csv", "r");
	var data = std.fread(f);
	std.fclose(f);
	
	var csv = csv.parse(data, ",", true);
	
	println("Rows: " + csv.row_count(csv));
	println("Columns: " + csv.col_count(csv));	

	csv.set(csv, "0.HistoricalPrice", 111);

	var headers = csv.headers(csv);
	println("Headers: " + csv.headers(csv));
	
	var it = csv.iter(csv);
	while(csv.next(it)) { 
	
		var row = csv.value(it);
		var i = 0;
